...
import { AvatarDanceModal } from "./AvatarDance";
import { DEFAULT_OUTFIT } from "./avatarConfig_updated";
import { AvatarSVG } from "./AvatarSVG";
import AvatarStore3D from "./AvatarStore3D_Realistic";
...
function TaskTimer({taskName,onClose}){
  ...
  useEffect(()=>{if(running){timerRef.current=setInterval(()=>{setSeconds(s=>{if(s<=1){clearInterval(timerRef.current);setRunning(false);toast("انتهى الوقت! 🎉");return 0;}return s-1;})},[]);}else clearInterval(timerRef.current);return()=>clearInterval(timerRef.current);},[running]);
...
const QUEST_STORAGE = "maha_quest_claims_v1";
function readQuestClaims(){
  try{ return JSON.parse(localStorage.getItem(QUEST_STORAGE) || "{}"); }catch{ return {}; }
}
function writeQuestClaims(obj){
  try{ localStorage.setItem(QUEST_STORAGE, JSON.stringify(obj)); }catch{}
}

const STORE_QUESTS = [
  { id:"sq_d1",  reset:"daily",   title:"بداية اليوم",       desc:"أنجزي مهمة واحدة اليوم",           need:c=>c.todayDone>=1,  reward:"+12⭐", badge:"🌅 شارة الصباح" },
  { id:"sq_d3",  reset:"daily",   title:"ثلاثية قوية",       desc:"٣ مهام منجزة اليوم",                need:c=>c.todayDone>=3,  reward:"+28⭐", badge:"⚡ شارة الحماس" },
  { id:"sq_d5",  reset:"daily",   title:"يوم أسطوري",      desc:"٥ مهام منجزة اليوم",                need:c=>c.todayDone>=5,  reward:"+55⭐", badge:"🔥 وسام اليوم" },
  { id:"sq_w10", reset:"weekly",  title:"أسبوع منتج",      desc:"١٠ مهام منجزة هذا الأسبوع",          need:c=>c.weekDone>=10,  reward:"+45⭐", badge:"📅 وسام الأسبوع" },
  { id:"sq_h5",  reset:"once",    title:"صائدة العاجل",    desc:"٥ مهام عاجلة منجزة (إجمالي)",       need:c=>c.highDone>=5,   reward:"+40⭐", badge:"🔴 وسام العزيمة" },
  { id:"sq_xp",  reset:"once",    title:"نجوم متراكمة",    desc:"وصلي ٣٠٠⭐ من إنجاز المهام",        need:c=>c.totalXP>=300,  reward:"+50⭐", badge:"💎 وسام النجوم" },
  { id:"sq_sub", reset:"weekly",  title:"تفصيل ذكي",       desc:"أكملي ٨ مهام فرعية (أسبوعياً)",      need:c=>c.weekSubDone>=8,reward:"+30⭐", badge:"🧩 شارة الدقة" },
];

function questKey(q, weekStart){
  if(q.reset==="daily") return `${q.id}_${new Date().toDateString()}`;
  if(q.reset==="weekly") return `${q.id}_w${weekStart.getTime()}`;
  return q.id;
}
  ...
  return(<div style={{background:isDone?"rgba(255,255,255,.02)":isDoing?"rgba(180,142,245,.05)":C.card,borderRadius:16,
    ...
    {isExpanded&&<div style={{borderTop:`1px solid ${C.border}`,padding:"12px 14px",background:"rgba(0,0,0,.12)"}}>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
        {col.id==="doing"&&<button onClick={()=>setTimerTask(t)} style={{display:"flex",alignItems:"center",gap:4,padding:"6px 12px",borderRadius:20,fontSize:11,fontFamily:"'Cairo',sans-serif",fontWeight:600,background:"rgba(180,142,245,.12)",color:C.purple,cursor:"pointer",border:"none"}}>⏱ تايمر</button>}
        {col.id!=="todo"&&<button onClick={()=>moveTask(t,"todo")} style={{padding:"6px 12px",borderRadius:20,fontSize:11,fontFamily:"'Cairo',sans-serif",cursor:"pointer",fontWeight:600,background:C.surface,border:`1px solid ${C.border}`,color:C.muted}}>← قائمة</button>}
        {col.id!=="doing"&&<button onClick={()=>moveTask(t,"doing")} style={{padding:"6px 12px",borderRadius:20,fontSize:11,fontFamily:"'Cairo',sans-serif",cursor:"pointer",fontWeight:600,background:"rgba(251,184,108,.12)",border:"1px solid rgba(251,184,108,.2)",color:C.warning}}>⚡ جاري</button>}
        {col.id!=="done"&&<button onClick={()=>moveTask(t,"done")} style={{display:"flex",alignItems:"center",gap:5,padding:"6px 14px",borderRadius:20,fontSize:11,fontFamily:"'Cairo',sans-serif",fontWeight:600,background:"rgba(110,231,183,.12)",border:"1px solid rgba(110,231,183,.2)",color:C.success,cursor:"pointer"}}>✅ انجزت</button>}
...
export default function Tasks({subjects}){
  ...
  const[questClaims,setQuestClaims]=useState(readQuestClaims);
  ...
  const computed=useMemo(()=>{
    ...
    const weekSubDone=done.filter(t=>{const d=parse(t.doneAt);return d&&d>=weekStart;})
      .reduce((s,t)=>s+(t.subtasks||[]).filter(x=>x.done).length,0);
    return{doneTasks:done,totalDone:done.length,
      ...
      datedDone:tasks.filter(t=>t.dueDate).length,weekSubDone};
  },[tasks,getStatus,PRI]);
  const{totalDone,todayDone,weekDone,highDone,totalXP,datedDone,weekSubDone}=computed;
  const today=useMemo(()=>{const d=new Date();d.setHours(0,0,0,0);return d;},[]);
  const weekStartQuest=useMemo(()=>{const d=new Date();d.setHours(0,0,0,0);d.setDate(d.getDate()-d.getDay());return d;},[]);
  ...
  const questBadgeUnlocked=useMemo(()=>STORE_QUESTS.filter(q=>questClaims[questKey(q,weekStartQuest)]).length,[questClaims,weekStartQuest]);

  const claimQuest=useCallback((q)=>{
    const ctx={todayDone,weekDone,highDone,totalXP,weekSubDone};
    if(!q.need(ctx)){toast("كمّلي المتطلبات الأول 💪","info");return;}
    const k=questKey(q,weekStartQuest);
    if(questClaims[k]){toast("مستلمة مسبقاً ✨","info");return;}
    const next={...questClaims,[k]:{badge:q.badge,title:q.title,reward:q.reward,at:Date.now()}};
    setQuestClaims(next);
    writeQuestClaims(next);
    toast(`🎁 ${q.reward} — ${q.badge}`);
  },[todayDone,weekDone,highDone,totalXP,weekSubDone,questClaims,weekStartQuest]);
  ...
  return(<div style={{position:"relative"}}>
    <style>{`@keyframes slideUp{from{transform:translateY(100%);opacity:0}to{transform:translateY(0);opacity:1}}@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}}@keyframes fadeUp{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}@keyframes spin{to{transform:rotate(360deg)}}` }</style>
    ...
    <Modal show={!!editTask} onClose={()=>setEditTask(null)} title="✎ تعديل المهمة">
      {editTask&&<div style={{display:"flex",flexDirection:"column",gap:11}}>
        <input value={editTask.text} onChange={e=>setEditTask(p=>({...p,text:e.target.value}))} autoFocus style={{background:C.surface,border:`1px solid ${C.border}`,color:C.text,padding:"9px 13px",borderRadius:8,fontFamily:"'Cairo',sans-serif",fontSize:13}}/>
        <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
          <select value={editTask.subject||""} onChange={e=>setEditTask(p=>({...p,subject:e.target.value}))} style={{flex:1,fontSize:12,background:C.surface,border:`1px solid ${C.border}`,color:C.text,borderRadius:8,fontFamily:"'Cairo',sans-serif",padding:"8px"}}>
            <option value="">بدون مادة</option>{subjects.map(s=><option key={s.id} value={s.name}>{s.icon} {s.name}</option>)}</select>
          <select value={editTask.priority||"medium"} onChange={e=>setEditTask(p=>({...p,priority:e.target.value}))} style={{width:"auto",fontSize:12,background:C.surface,border:`1px solid ${C.border}`,color:C.text,borderRadius:8,fontFamily:"'Cairo',sans-serif",padding:"8px"}}>
      ...
      <div><h2 style={{fontSize:20,fontWeight:900,color:C.text,marginBottom:3,fontFamily:"'Cairo',sans-serif"}}>المهام</h2>
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {[{v:stats.todo,label:"قائمة",color:C.accent},{v:stats.doing,label:"جاري",color:C.warning},{v:stats.done,label:"منجزة",color:C.success},{v:weekDone,label:"الأسبوع",color:C.purple}]
            .map((s,i)=>(<span key={i} style={{fontSize:11,color:s.color,fontWeight:700,background:s.color+"18",padding:"2px 9px",borderRadius:20,border:"1px solid "+s.color+"25",fontFamily:"'Cairo',sans-serif"}}>{s.v} {s.label}</span>))}
        </div>
      </div>
    ...
    {nextM&&<div style={{marginBottom:14,borderRadius:14,overflow:"hidden",cursor:"pointer",background:"linear-gradient(135deg,rgba(180,142,245,.12),rgba(244,201,122,.06))",border:"1px solid rgba(180,142,245,.2)",padding:"14px 15px"}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:10,color:"rgba(245,238,248,.5)",lineHeight:1.6,fontFamily:"'Cairo',sans-serif"}}>{nextM-totalDone===1?"مهمة واحدة بس!":`${nextM-totalDone} مهام متبقية`}</div>
        </div>
    ...
    <div style={{display:"flex",borderBottom:`1px solid ${C.border}`,marginBottom:14,flexWrap:"wrap",gap:2}}>
      {[{id:"board",label:"📋 المهام"},{id:"quests",label:`🎯 مكافآت (${questBadgeUnlocked}/${STORE_QUESTS.length})`},{id:"rewards",label:`⭐ النجوم (${unlockedIds.length}/${ACHIEVEMENTS.length})`}]
        .map(tab=>(<button key={tab.id} onClick={()=>setActiveTab(tab.id)} style={{flex:1,minWidth:72,padding:"9px 4px",fontSize:10,fontFamily:"'Cairo',sans-serif",fontWeight:700,cursor:"pointer",border:"none",background:"transparent",color:activeTab===tab.id?C.accent:C.muted,borderBottom:activeTab===tab.id?`2px solid ${C.accent}`:"none",transition:"all .2s"}}>{tab.label}</button>))}</div>
    ...
    {activeTab==="board"&&<div style={{display:"flex",gap:6,marginBottom:14,overflowX:"auto",paddingBottom:2}}>
      {[{v:"all",l:"الكل",c:C.accent},{v:"high",l:"🔴 عاجل",c:C.danger},{v:"medium",l:"🟡 متوسط",c:C.warning},{v:"low",l:"🟢 عادي",c:C.success}]
        .map(f=>(<button key={f.v} onClick={()=>setFilterPri(filterPri===f.v?"all":f.v)} style={{flexShrink:0,padding:"5px 14px",borderRadius:20,fontSize:11,fontFamily:"'Cairo',sans-serif",fontWeight:700,background:filterPri===f.v?f.c+"22":C.surface,border:`1px solid ${filterPri===f.v?f.c:C.border}`,color:filterPri===f.v?f.c:C.muted,cursor:"pointer"}}>{f.l}</button>))}
      <select value={filterSubj} onChange={e=>setFilterSubj(e.target.value)} style={{flexShrink:0,fontSize:11,padding:"4px 10px",width:"auto",borderRadius:20,background:C.surface,border:`1px solid ${C.border}`,color:C.text,fontFamily:"'Cairo',sans-serif",cursor:"pointer"}}>
        <option value="all">📚 كل المواد</option>{subjects.map(s=><option key={s.id} value={s.name}>{s.icon} {s.name}</option>)}</select>
      <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{flexShrink:0,fontSize:11,padding:"4px 10px",width:"auto",borderRadius:20,background:C.surface,border:`1px solid ${C.border}`,color:C.text,fontFamily:"'Cairo',sans-serif",cursor:"pointer"}}>
    ...
    {showAdd&&<div style={{marginBottom:16,padding:"16px 15px",borderRadius:18,background:C.card,border:"1.5px solid rgba(232,121,160,.35)",boxShadow:"0 8px 32px rgba(232,121,160,.08)"}}>
      <div style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:8}}>{Object.entries(PRI).map(([k,v])=>(<button key={k} onClick={()=>setNewPri(k)} style={{flex:1,padding:"11px 6px",borderRadius:12,border:"none",cursor:"pointer",background:newPri===k?v.bg:C.surface,border:`1px solid ${newPri===k?v.color:C.border}`,fontFamily:"'Cairo',sans-serif",transition:"all .2s"}}>
          <div style={{fontSize:20,marginBottom:4}}>{v.icon}</div><div style={{fontSize:12,fontWeight:700,color:newPri===k?v.color:C.muted}}>{v.label}</div><div style={{fontSize:10,color:newPri===k?v.color:C.dim}}>+{v.pts}⭐</div></button>))}
        </div>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:9}}>
        <input value={newText} onChange={e=>setNewText(e.target.value)} placeholder="✦ اكتبي المهمة هنا..." autoFocus style={{background:C.surface,border:`1px solid ${C.border}`,color:C.text,padding:"9px 13px",borderRadius:8,fontFamily:"'Cairo',sans-serif",fontSize:13}}/>
        <div style={{display:"flex",gap:7}}>
          <select value={newSubj} onChange={e=>setNewSubj(e.target.value)} style={{flex:1,fontSize:12,background:C.surface,border:`1px solid ${C.border}`,color:C.text,padding:"7px 10px",borderRadius:6,fontFamily:"'Cairo',sans-serif"}}>
            <option value="">📚 بدون مادة</option>{subjects.map(s=><option key={s.id} value={s.name}>{s.icon} {s.name}</option>)}</select>
          <input type="date" value={newDate} onChange={e=>setNewDate(e.target.value)} style={{width:"auto",fontSize:12,padding:"7px 9px",background:C.surface,border:`1px solid ${C.border}`,color:C.text,borderRadius:6,fontFamily:"'Cairo',sans-serif"}}/>
        </div>
        <textarea value={newNote} onChange={e=>setNewNote(e.target.value)} rows={2} placeholder="💬 ملاحظة اختيارية..." style={{background:C.surface,border:`1px solid ${C.border}`,color:C.text,padding:"9px 13px",borderRadius:8,fontFamily:"'Cairo',sans-serif",fontSize:12,resize:"vertical"}}/>
        <textarea value={newSubs} onChange={e=>setNewSubs(e.target.value)} rows={3} placeholder={"قراءة الملخص\nحل الأسئلة\nمراجعة الدرس"} style={{background:C.surface,border:`1px solid ${C.border}`,color:C.text,padding:"9px 13px",borderRadius:8,fontFamily:"'Cairo',sans-serif",fontSize:12,resize:"vertical"}}/>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 13px",borderRadius:11,background:"rgba(244,201,122,.07)",border:"1px solid rgba(244,201,122,.18)"}}>
          ...
          <div style={{display:"flex",gap:7}}><Btn variant="secondary" onClick={()=>setShowAdd(false)}>إلغاء</Btn><Btn onClick={add} disabled={!newText.trim()}>إضافة ✦</Btn></div></div>
      </div>
    </div>}
    ...
    :activeTab==="board"?<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,alignItems:"start"}} className="kanban-board">
      {COLS.map(col=>{const colTasks=getColTasks(col.id);return(<div key={col.id} style={{borderRadius:16,overflow:"hidden",border:"1px solid "+C.border,background:C.bg,display:"flex",flexDirection:"column"}}>
        <div style={{padding:"11px 14px",background:col.glow,borderBottom:"1px solid "+C.border,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <span style={{fontSize:11,fontWeight:800,color:col.color,background:col.color+"15",padding:"2px 9px",borderRadius:20,border:"1px solid "+col.color+"25",fontFamily:"'Cairo',sans-serif"}}>{colTasks.length}</span>
        </div>
    ...
    :activeTab==="quests"?<div style={{paddingBottom:16}}>
      <div style={{fontSize:12,color:C.muted,marginBottom:14,lineHeight:1.75,fontFamily:"'Cairo',sans-serif",background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"12px 14px"}}>
        أنجزي المهام في اللوحة؛ عند تحقق الشرط اضغطي <b style={{color:C.text}}>استلام</b> لتسجيل الشارة (محلياً على جهازك). الشارات تساعدك تتابعي تطورك! 🎁
      </div>
      {STORE_QUESTS.map(q=>{
        const ctx={todayDone,weekDone,highDone,totalXP,weekSubDone};
        const met=q.need(ctx);
        const k=questKey(q,weekStartQuest);
        const claimed=!!questClaims[k];
        const resetLabel=q.reset==="daily"?"يومي":q.reset==="weekly"?"أسبوعي":"مرة واحدة";
        return(<div key={q.id} style={{marginBottom:10,borderRadius:16,background:claimed?"rgba(110,231,183,.06)":C.card,border:`1px solid ${claimed?"rgba(110,231,183,.35)":met?C.purple+"44":C.border}`,padding:"12px 14px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,marginBottom:6}}>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:10,color:C.dim,marginBottom:3,fontFamily:"'Cairo',sans-serif"}}>{resetLabel}</div>
              <div style={{fontSize:14,fontWeight:800,color:C.text,fontFamily:"'Cairo',sans-serif"}}>{q.title}</div>
              <div style={{fontSize:11,color:C.muted,marginTop:4,lineHeight:1.6,fontFamily:"'Cairo',sans-serif"}}>{q.desc}</div>
            </div>
            <div style={{textAlign:"center",flexShrink:0}}>
              <div style={{fontSize:11,fontWeight:800,color:C.gold,fontFamily:"'Cairo',sans-serif"}}>{q.reward}</div>
              <div style={{fontSize:22,marginTop:2,lineHeight:1}}>{claimed?"🏅":met?"✨":"🔒"}</div>
            </div>
          </div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
            <span style={{fontSize:10,color:met?C.success:C.dim,fontFamily:"'Cairo',sans-serif",fontWeight:600}}>{met?"✅ الشرط متحقق":"⏳ لسه على الطريق"}</span>
            <Btn onClick={()=>claimQuest(q)} disabled={!met||claimed} variant={claimed?"success":"primary"} style={{padding:"6px 16px",fontSize:12}}>{claimed?"تم الاستلام":"استلام"}</Btn>
          </div>
          {claimed&&<div style={{fontSize:10,color:C.success,marginTop:8,fontFamily:"'Cairo',sans-serif"}}>🏅 {questClaims[k]?.badge||q.badge}</div>}
        </div>);
      })}
    </div>

    :activeTab==="rewards"?<div style={{paddingBottom:16}}>
      <div style={{marginBottom:12}}>
        ...
        {unlockedIds.length > 0 && <div style={{marginBottom:8}}>
          <div style={{fontSize:10,color:C.success,fontWeight:700,fontFamily:"'Cairo',sans-serif",marginBottom:6,padding:"3px 10px",background:"rgba(125,211,176,.08)",borderRadius:20,display:"inline-block",border:"1px solid rgba(125,211,176,.2)"}}>
            ✅ مفتوح ({unlockedIds.length})
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(90px,1fr))",gap:7}}>
            {ACHIEVEMENTS.filter(a=>unlockedIds.includes(a.id)).map(a=>(<div key={a.id} style={{padding:"12px 7px",borderRadius:14,textAlign:"center",border:"1px solid rgba(110,231,183,.4)",background:"rgba(110,231,183,.08)"}}>
        ...
        {ACHIEVEMENTS.filter(a=>!unlockedIds.includes(a.id)).length > 0 && <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(90px,1fr))",gap:7}}>
            {ACHIEVEMENTS.filter(a=>!unlockedIds.includes(a.id)).map(a=>(<div key={a.id} style={{padding:"12px 7px",borderRadius:14,textAlign:"center",border:`1px solid ${C.border}`,background:C.surface}}>
    ...
    :<AvatarStore3D totalXP={totalXP} onEquipChange={setOutfit}/>}
  </div>);
}
