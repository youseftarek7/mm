...
/** مهام مكافآت وأوسمة (تقدم من إنجازك الفعلي؛ الاستلام يخزّن شارة محلياً) */
const STORE_QUESTS = [
  ...
  { id:"sq_h5",  reset:"once",    title:"صائدة العاجل",    desc:"٥ مهام عاجلة منجزة (إجمالي)",       need:c=>c.highDone>=5,   reward:"+40⭐", badge:"🔴 وسام الأولوية" },
  { id:"sq_xp",  reset:"once",    title:"نجوم متراكمة",    desc:"وصلي ٣٠٠⭐ من إنجاز المهام",        need:c=>c.totalXP>=300,  reward:"+50⭐", badge:"💎 وسام النجوم" },
  { id:"sq_sub", reset:"weekly",  title:"تفصيل ذكي",       desc:"أكملي ٨ مهام فرعية (أسبوعياً)",      need:c=>c.weekSubDone>=8,reward:"+30⭐", badge:"🧩 شارة التفصيل" },
];
...
export default function Tasks({subjects}){
  ...
  return(<div style={{position:"relative"}}>
    ...
    <div style={{display:"flex",borderBottom:`1px solid ${C.border}`,marginBottom:14,flexWrap:"wrap",gap:2}}>
      {[{id:"board",label:"📋 المهام"},{id:"quests",label:`🎯 مكافآت (${questBadgeUnlocked}/${STORE_QUESTS.length})`},{id:"rewards",label:`⭐ النجوم (${unlockedIds.length}/${ACHIEVEMENTS.length})`},{id:"store",label:"🛍️ المتجر"}]
    ...
    :activeTab==="store"?<AvatarStore3D totalXP={totalXP} onEquipChange={setOutfit}/>
    ...
    :activeTab==="quests"?<div style={{paddingBottom:16}}>
      <div style={{fontSize:12,color:C.muted,marginBottom:14,lineHeight:1.75,fontFamily:"'Cairo',sans-serif",background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"12px 14px"}}>
        أنجزي المهام في اللوحة؛ عند تحقق الشرط اضغطي <b style={{color:C.text}}>استلام</b> لتسجيل الشارة (محلياً على جهازك). النجوم هنا تحفيز — رصيدك الفعلي يزيد كالعادة عند إتمام المهام في «تم الإنجاز» ⭐
      </div>
      {STORE_QUESTS.map(q=>{
        const ctx={todayDone,weekDone,highDone,totalXP,weekSubDone};
        ...
        return(<div key={q.id} style={{marginBottom:10,borderRadius:16,background:claimed?"rgba(110,231,183,.06)":C.card,border:`1px solid ${claimed?"rgba(110,231,183,.35)":met?C.purple+"44":C.border}`,padding:"13px 14px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,marginBottom:6}}>
            ...
            <div style={{textAlign:"center",flexShrink:0}}>
              ...
              <div style={{fontSize:22,marginTop:2,lineHeight:1}} title={q.badge}>{claimed?"🏅":met?"✨":"🔒"}</div>
            </div>
          </div>
